...
First make sure you have sphinx installed as well as the sphinxawesometheme
...
1. run_adam.py
   ...
   :ref:`ADAM Control Module <ADAM_control_module>` is called to read the output and update the parameters.
   Here is where one could make edits in order to change the machine that the MC solver is running on.
...
This module is called to update the parameters for each step of the gradient descent. 
